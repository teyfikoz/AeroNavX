...
def midpoint(lat1: float, lon1: float, lat2: float, lon2: float) -> tuple[float, float]:
    ...
    lat_mid = math.atan2(
        math.sin(lat1_rad) + math.sin(lat2_rad), math.sqrt((math.cos(lat1_rad) + bx) ** 2 + by**2)
    )
...
def intermediate_point(
    ...
    x = a_frac * math.cos(lat1_rad) * math.cos(lon1_rad) + b_frac * math.cos(lat2_rad) * math.cos(
        lon2_rad
    )
    y = a_frac * math.cos(lat1_rad) * math.sin(lon1_rad) + b_frac * math.cos(lat2_rad) * math.sin(
        lon2_rad
    )
